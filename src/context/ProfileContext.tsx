...
export const ProfileProvider: React.FC<ProfileProviderProps> = ({
  ...
  const loadProfile = async (userId: string) => {
    try {
      ...
      if (loadedProfile) {
        ...
        const changed =
          JSON.stringify(loadedProfile) !== JSON.stringify(normalized);
        if (changed) {
  ...
  const removeChild = async (childId: string): Promise<boolean> => {
    ...
    const updatedChildren =
      clientProfile.children?.filter((child) => child.id !== childId) || [];
    const updatedProfile: Client = {
      ...clientProfile,
      children: updatedChildren,
    };
  ...
  const removePaymentMethod = async (
    ...
    const clientProfile = profile as Client;
    const updatedPaymentMethods =
      clientProfile.paymentMethods?.filter(

...
export const getRideHistoryByPeriod = (
...
export const getHourlyActivityByPeriod = (
  period: "today" | "week" | "month" | "year",
) => {
  switch (period) {
    case "today":
...
export const getDistanceMetricsByPeriod = (
  period: "today" | "week" | "month" | "year",
) => {
  ...
  switch (period) {
    ...
    case "week":
      return baseMetrics.map((metric) => ({
        ...
        value:
          period === "week"
            ? metric.metric === "Общий пробег"
              ? "420 км"
              : metric.metric === "Время в пути"
                ? "12ч 15мин"
                : metric.value
            : metric.value,
      }));
...
export const getRoutePointsByPeriod = (
  period: "today" | "week" | "month" | "year",
) => {
  switch (period) {
    case "today":

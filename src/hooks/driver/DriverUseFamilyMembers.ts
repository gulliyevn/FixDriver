...
export const useDriverFamilyMembers = () => {
  const { t } = useI18n();
  ...
  const loadFamilyMembers = useCallback(async () => {
    try {
      const savedMembers = await AsyncStorage.getItem('driver_family_members');
      if (savedMembers) {
        ...
        await AsyncStorage.setItem('driver_family_members', JSON.stringify(initialMembers));
      }
    } catch (error) {
  ...
  const saveFamilyMembers = useCallback(async (members: FamilyMember[]) => {
    try {
      await AsyncStorage.setItem('driver_family_members', JSON.stringify(members));
    } catch (error) {

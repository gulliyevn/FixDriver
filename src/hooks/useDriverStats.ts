...
import DriverStatsService, {
  DayStats,
  PeriodStats,
} from "../services/DriverStatsService";
...
export const useDriverStats = () => {
  ...
  const getPeriodStats = useCallback(
    async (
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Ошибка получения статистики";
  ...
  const getLastNDaysStats = useCallback(
    async (days: number): Promise<DayStats[]> => {
      setLoading(true);
      setError(null);
  ...
  const getDayStats = useCallback(
    async (date: string): Promise<DayStats | null> => {
      setLoading(true);
      setError(null);
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Ошибка получения статистики дня";
        setError(errorMessage);
        return null;
      } finally {
        setLoading(false);

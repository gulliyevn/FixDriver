...
export const useAddresses = (): UseAddressesReturn => {
  ...
  const loadAddresses = useCallback(async () => {
    ...
    try {
      if (__DEV__) {
        ...
        if (user?.id) {
          const loadedAddresses = await addressServiceInstance.getAddresses(
            user.id,
          );
  ...
  const addAddress = useCallback(
    async (addressData: CreateAddressRequest): Promise<boolean> => {
      try {
        if (__DEV__) {
          ...
          const newAddress = await addressServiceInstance.createAddress(
            user.id,
            addressData,
          );
          if (newAddress) {
  ...
  const updateAddress = useCallback(
    async (id: string, updates: UpdateAddressRequest): Promise<boolean> => {
      try {
        if (__DEV__) {
      ...
      try {
        ...
        if (__DEV__) {
          ...
          const updated = addresses.map((addr) => ({
            ...
            isDefault:
              addr.id === id ||
              (addr.category === targetAddress.category &&
              addr.isDefault &&
              addr.id !== id
                ? false
                : addr.isDefault),
          ...
          // PROD: обновляем через API
          if (!user?.id) return false;
          const success = await addressServiceInstance.setDefaultAddress(
            user.id,
            id,
          );
          if (success) {

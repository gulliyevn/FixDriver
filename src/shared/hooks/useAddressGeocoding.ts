...
export const useAddressGeocoding = (addressService?: AddressService) => {
  const serviceRef = useMemo(() => addressService ?? new AddressService(), [addressService]);
  ...
  const geocodeFromCoordinates = useCallback(
    async (coordinates: Coordinates) => {
      ...
      try {
        const response = await serviceRef.reverseGeocode(coordinates.latitude, coordinates.longitude);

...
describe("Validators", () => {
  describe("validateEmail", () => {
    ...
    it("validates email length", () => {
      ...
      expect(result.errors.some((error) => error.includes("превышать"))).toBe(
        true,
      );
    });
    ...
    it("warns about temporary email services", () => {
      ...
      expect(result.isValid).toBe(true);
      expect(
        result.warnings?.some((warning) => warning.includes("постоянный")),
      ).toBe(true);
    });
    ...
    it("handles empty email", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors.some((error) => error.includes("обязателен"))).toBe(
        true,
      );
    });
  ...
  describe("validatePassword", () => {
    ...
    it("warns about weak passwords", () => {
      ...
      expect(
        result.warnings?.some((warning) => warning.includes("слабый")),
      ).toBe(true);
    });
  ...
  describe("validateName", () => {
    ...
    it("validates with custom field name", () => {
      ...
      expect(result.errors.some((error) => error.includes("Фамилия"))).toBe(
        true,
      );
    });
  });
  ...
  describe("validateLicenseExpiry", () => {
    ...
    it("rejects invalid date formats", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(
        result.errors.some((error) => error.includes("Некорректная дата")),
      ).toBe(true);
    });
  });
  ...
  describe("validateClientRegistration", () => {
    ...
    it("validates password confirmation", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(
        result.errors.some((error) => error.includes("не совпадают")),
      ).toBe(true);
    });
  });

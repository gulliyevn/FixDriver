...
describe("AppAvatar Component", () => {
  ...
  it("renders with initials when no image", () => {
    const { getByText } = render(<AppAvatar name="John Doe" size={50} />);

    expect(getByText("JD")).toBeTruthy();
  });

  it("renders with single name", () => {
    const { getByText } = render(<AppAvatar name="John" size={50} />);
  ...
  it("handles empty name gracefully", () => {
    const { getByText } = render(<AppAvatar name="" size={50} />);

    expect(getByText("?")).toBeTruthy();
  });

  it("handles special characters in name", () => {
    const { getByText } = render(<AppAvatar name="José María" size={50} />);

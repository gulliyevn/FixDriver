...
const EarningsScreen: React.FC = () => {
  ...
  const levelTranslations =
    levelsTranslations[currentLanguage as keyof typeof levelsTranslations] ||
    levelsTranslations.ru;
  ...
  const isQualifiedToday =
    driverLevel.isVIP && vipCurrentHours >= 10 && vipRidesToday >= 3;
  ...
  const baseMonthlyPreview = driverLevel.isVIP
    ? displayQualifiedDays < 20
      ? 0
      : displayQualifiedDays <= 25
        ? VIP_CONFIG.monthlyBonuses.days20
        : displayQualifiedDays <= 29
          ? VIP_CONFIG.monthlyBonuses.days25
  ...
  if (driverLevel.isVIP) {
    ...
    if (qualifiesCurrent) {
      if (effectiveConsecutive === 3)
        quarterlyPreview = VIP_CONFIG.quarterlyBonuses.months3;
      else if (effectiveConsecutive === 6)
        quarterlyPreview = VIP_CONFIG.quarterlyBonuses.months6;
      else if (effectiveConsecutive === 12)
        quarterlyPreview = VIP_CONFIG.quarterlyBonuses.months12;
    }
  }
  ...
  const generateLevelModalContent = () => {
    ...
    for (let level = 1; level <= 6; level++) {
      ...
      if (levelConfigs[0]) {
        const levelName = getTranslatedLevelName(
          levelConfigs[0].levelKey,
          1,
          t,
        ).split(" ")[0]; // Берем только название уровня без номера
              ...
              <View style={styles.earningsLeft}>
                ...
                <View>
                  <View style={{ flexDirection: "row", alignItems: "center" }}>
                    ...
                    {driverLevel.isVIP ? (
                      <Text
                        style={[styles.earningsReward, { color: "#10B981" }]}
                      >
                        +{vipMonthlyPreviewBonus} AFc
                      </Text>
                    ) : (
                      <Text style={styles.earningsReward}>
                        +{parseInt(driverLevel.nextReward)} AFc
                      </Text>
                    )}
                  </View>
                  <Text
              ...
              <Text style={styles.modalTimerText}>
                {(() => {
                  ...
                  const totalSeconds = Math.max(
                    0,
                    Math.floor(vipCurrentHours * 3600),
                  );
                  const hh = String(Math.floor(totalSeconds / 3600)).padStart(
                    2,
                    "0",
                  );
                  const mm = String(
      ...
      >
        <View style={styles.modalOverlay}>
          <View
            style={[
              styles.modalContent,
              { maxHeight: "80%", width: "90%", minHeight: 500 },
            ]}
          >
              ...
              <Text style={[styles.modalMessage, { textAlign: "left" }]}>
                {generateLevelModalContent()
                  ...
                  .map((part, index) => {
                    ...
                    if (
                      part.includes(
                        "Ежеквартальными и Ежемесячными бонусами",
                      ) ||

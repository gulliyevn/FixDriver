...
export const useVIPTimeTracking = (isVIP: boolean) => {
  ...
  const performDayCheck = useCallback(async () => {
    ...
    if (vipTimeData.vipCycleStartDate) {
      ...
      const diffDays = Math.floor(
        (now.getTime() - cycleStart.getTime()) / msInDay,
      );
      if (diffDays >= 360) {
    ...
    if (today !== vipTimeData.currentDay) {
      ...
      const isQualifiedDay =
        effectiveHours >= MIN_HOURS_PER_DAY &&
        vipTimeData.ridesToday >= VIP_CONFIG.minRidesPerDay;
    ...
    if (isVIP && vipTimeData.periodStartDate) {
      ...
      const msInDay = 24 * 60 * 60 * 1000;
      const diffDays = Math.floor(
        (now.getTime() - periodStart.getTime()) / msInDay,
      );
      if (diffDays >= 30) {
        ...
        const metMonthlyRequirement =
          qualifiedDays >= VIP_CONFIG.minDaysPerMonth;
        let trailingQualified = 0;
        for (let i = newHistory.length - 1; i >= 0; i -= 1) {
          if (newHistory[i] >= VIP_CONFIG.minDaysPerMonth)
            trailingQualified += 1;
          else break;
        }
        ...
        const shouldResetCycle =
          !metMonthlyRequirement || trailingQualified === 12;
        ...
        const newData: VIPTimeData = shouldResetCycle
          ...
          : {
              ...
              vipCycleStartDate:
                trailingQualified === 1
                  ? new Date(vipTimeData.currentMonth + "-01T00:00:00")
                      .toISOString()
                      .split("T")[0]
  ...
  const loadVIPTimeData = async () => {
    try {
      ...
      if (saved) {
        ...
        parsed.qualifiedDaysThisMonth = Math.max(
          0,
          Math.min(30, parsed.qualifiedDaysThisMonth || 0),
        );
  ...
  const getCurrentHoursOnline = useCallback(() => {
    ...
    if (vipTimeData.isCurrentlyOnline && vipTimeData.lastOnlineTime) {
      const currentSessionHours =
        (now - vipTimeData.lastOnlineTime) / (1000 * 60 * 60);
  ...
  const simulateDayChange = useCallback(async () => {
    const isQualified =
      vipTimeData.hoursOnline >= MIN_HOURS_PER_DAY &&
      vipTimeData.ridesToday >= VIP_CONFIG.minRidesPerDay;
    ...
    const newData: VIPTimeData = {
      ...
      hoursOnline: 0,
      ridesToday: 0,
      qualifiedDaysThisMonth:
        vipTimeData.qualifiedDaysThisMonth + (isQualified ? 1 : 0),
      lastOnlineTime: null,
      isCurrentlyOnline: false,
    };
  ...
  const simulateMonthChange = useCallback(async () => {
    ...
    else if (qualifiedDays >= 25)
      monthlyBonus = VIP_CONFIG.monthlyBonuses.days25;
    else if (qualifiedDays >= 20)
      monthlyBonus = VIP_CONFIG.monthlyBonuses.days20;

    // Обновляем счетчик последовательных месяцев
    const metMonthlyRequirement = qualifiedDays >= 20;
    const newConsecutive = metMonthlyRequirement
      ? vipTimeData.consecutiveQualifiedMonths + 1
      : 0;
    ...
    if (metMonthlyRequirement) {
      if (newConsecutive === 3)
        quarterlyBonus = VIP_CONFIG.quarterlyBonuses.months3;
      else if (newConsecutive === 6)
        quarterlyBonus = VIP_CONFIG.quarterlyBonuses.months6;
      else if (newConsecutive === 12)
        quarterlyBonus = VIP_CONFIG.quarterlyBonuses.months12;
    }
    ...
    if (metMonthlyRequirement) {
      if (newConsecutive === 1) {
        ...
        const prevMonthDate = new Date(
          vipTimeData.currentMonth + "-01T00:00:00",
        );
    ...
    const newData: VIPTimeData = {
      ...vipTimeData,
      currentMonth: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000)
        .toISOString()
        .slice(0, 7),
      qualifiedDaysThisMonth: 0,
  ...
  const checkCurrentDayQualification = useCallback(() => {
    ...
    const isQualified =
      currentHours >= MIN_HOURS_PER_DAY &&

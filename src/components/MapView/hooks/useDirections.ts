...
const buildDirectionsUrl = (
  ...
  const rawWaypoints = points
    .slice(1, -1)
    .map((p) => `${p.coordinate.latitude},${p.coordinate.longitude}`);
  const waypointPrefix =
    opts.optimize && rawWaypoints.length > 0 ? "optimize:true|" : "";
  const waypoints =
    rawWaypoints.length > 0
      ? `&waypoints=${encodeURIComponent(waypointPrefix + rawWaypoints.join("|"))}`
      : "";
  const avoid = [
    opts.avoidTolls ? "tolls" : null,
    opts.avoidHighways ? "highways" : null,
  ]
    .filter(Boolean)
    .join("|");
  ...
  useEffect(() => {
    const fetchDirections = async () => {
      ...
      try {
        ...
        let url = buildDirectionsUrl(points!, {
          optimize: false,
          avoidTolls: true,
          avoidHighways: true,
        });
        ...
        if (
          ...
          (isLate || best.waypoint_order?.length > 0)
        ) {
          const urlOpt = buildDirectionsUrl(points!, {
            optimize: true,
            avoidTolls: true,
            avoidHighways: true,
          });
        ...
        const stillLate = plannedArrivalAtMs
          ? etaAfterOptMs > plannedArrivalAtMs
          : false;
        if (stillLate) {
          // Разрешаем платные
          const urlTolls = buildDirectionsUrl(points!, {
            optimize: true,
            avoidTolls: false,
            avoidHighways: false,
          });
          ...
          if (dataTolls.status === "OK" && dataTolls.routes?.length) {
            ...
            if (
              durTolls + tollsSpeedupThresholdMin * 60 <=
              recomputeDurationSec
            ) {
              best = bestTolls;
            }
          }
        ...
        setRoute({
          coordinates: finalDecoded,
          durationInTrafficSec: finalDurationSec,

...
async function writeLive(state: BillingLiveState): Promise<void> {
  try {
    await AsyncStorage.setItem(STORAGE_KEYS.LIVE, JSON.stringify(state));
  } catch (_e) {
    void 0;
  }
}
...
function calculateAmount(
  ...
  startedAt: number,
  endedAt: number,
): { chargedSeconds: number; amountAFc: number } {
  const totalSeconds = Math.max(0, Math.floor((endedAt - startedAt) / 1000));
  if (type === "waiting") {
    const billable = Math.max(0, totalSeconds - WAITING_FREE_SECONDS);
    return {
      chargedSeconds: billable,
      amountAFc: +(billable * PRICE_PER_SECOND_AFC).toFixed(2),
    };
  }
...
class BillingServiceImpl {
  ...
  async stopWaiting(): Promise<BillingRecord | null> {
    ...
    const endedAt = nowMs();
    const { chargedSeconds, amountAFc } = calculateAmount(
      "waiting",
      live.waitingStartedAt,
      endedAt,
    );
  ...
  async getLiveState(): Promise<BillingLiveState> {
    return readLive();
  }

...
class DriverService {
  ...
  static async getDriversPaged(
    params: DriverListParams,
  ): Promise<APIResponse<DriverListDto>> {
    return apiClient.get<DriverListDto>(
      "/drivers",
  ...
  private static async mockDriverRegistration(
    data: DriverRegistrationData,
  ): Promise<DriverRegistrationResponse> {
  ...
  static async updateDriverDocuments(
    driverId: string,
    data: DriverDocumentUpdateData,
  ): Promise<{ success: boolean; message: string }> {
    try {
      const response = await fetch(
        `${this.BASE_URL}/drivers/${driverId}/documents`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  static async getDriverStats(driverId: string): Promise<DriverStats> {
    try {
      const response = await fetch(
        `${this.BASE_URL}/drivers/${driverId}/stats`,
        {
          headers: {
            Authorization: `Bearer ${this.getToken()}`,
          },
  ...
  static async updateDriverLocation(
    location: Omit<DriverLocation, "driver_id">,
  ): Promise<void> {
    try {
      ...
      if (!response.ok) {
        console.error("Ошибка при обновлении местоположения");
        return;
      }
    } catch (error) {
      if (__DEV__) {
  ...
  static async getDriverTrips(driverId: string): Promise<DriverTrip[]> {
    try {
      const response = await apiClient.get<{ items: DriverTrip[] }>(
  ...
  private static mockUpdateDriverProfile(
    driverId: string,
    data: DriverUpdateData,
  ): Driver {
    return {

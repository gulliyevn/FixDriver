...
class PlacesService {
  ...
  validateAddress(address: string): { isValid: boolean; error?: string } {
    ...
    if (trimmed.length < 5) {
      return {
        isValid: false,
        error: "components:common.autocomplete.errors.tooShort",
      };
    }
    ...
    if (!hasNumber) {
      return {
        isValid: false,
        error: "components:common.autocomplete.errors.noNumber",
      };
    }

    // Проверяем наличие улицы (минимум 3 символа)
    const words = trimmed.split(/\s+/);
    const hasStreet = words.some(
      (word) => word.length >= 3 && !/\d/.test(word),
    );
    if (!hasStreet) {
      return {
        isValid: false,
        error: "components:common.autocomplete.errors.noStreet",
      };
    }

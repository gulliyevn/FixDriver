...
describe("NotificationService", () => {
    ...
    it("marks a specific notification as read", () => {
      ...
      if (unreadNotification) {
        ...
        const updatedNotification = service
          .getNotifications()
          .find((n) => n.id === unreadNotification.id);
  ...
  describe("removeNotification", () => {
    it("removes a specific notification", () => {
      ...
      expect(
        newNotifications.find((n) => n.id === notificationToRemove.id),
      ).toBeUndefined();
    });
  });
  ...
  describe("Async methods", () => {
    ...
    describe("markAllAsReadAsync", () => {
      it("marks all notifications as read for specific user", async () => {
        ...
        const userNotifications = service
          .getNotifications()
          .filter((n) => n.userId === "user1");
    ...
    describe("createNotificationAsync", () => {
      it("creates new notification asynchronously", async () => {
        ...
        const newNotification =
          await service.createNotificationAsync(notificationData);
  ...
  describe("getNotificationsByType", () => {
    it("returns notifications filtered by type", () => {
      const systemNotifications = service.getNotificationsByType(
        "user1",
        "system",
      );

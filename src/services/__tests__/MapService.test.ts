...
describe('MapService', () => {
  ...
  describe('cacheLocation', () => {
    ...
    it('handles caching error gracefully', async () => {
      ...
      await expect((MapService as any).cacheLocation(testLocation)).resolves.not.toThrow();
    });
  });
  ...
  describe('getCachedLocation', () => {
    it('returns cached location when valid', async () => {
      ...
      mockAsyncStorage.getItem.mockResolvedValue(JSON.stringify(mockCachedData));
    ...
    it('returns null when cache is expired', async () => {
      ...
      expect(mockAsyncStorage.removeItem).toHaveBeenCalledWith('cached_location');
    });
  ...
  describe('getCurrentLocation', () => {
    ...
    it('requests location permission when no cache', async () => {
      ...
      expect(result.longitude).toBe(49.8920);
    });
  ...
  describe('getRoute', () => {
    it('returns route between two locations', async () => {
      ...
      const mockRoute: MapLocation[] = [
        { latitude: 40.3777, longitude: 49.8920 },
  ...
  describe('getCurrentLocationWithRetry', () => {
    ...
    it('returns default region after all retries fail', async () => {
      mockAsyncStorage.getItem.mockResolvedValue(null);
      mockLocation.requestForegroundPermissionsAsync.mockResolvedValue({ status: 'granted' });
      mockLocation.getCurrentPositionAsync.mockRejectedValue(new Error('All attempts failed'));

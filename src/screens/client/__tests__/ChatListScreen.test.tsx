...
describe('ChatListScreen', () => {
  const mockChats = [
    {
      ...
      lastMessage: { content: 'Hello there!', timestamp: '2024-01-01T10:00:00Z' },
      unreadCount: 2,
    ...
    },
    {
      ...
      lastMessage: { content: 'How are you?', timestamp: '2024-01-01T09:00:00Z' },
      unreadCount: 0,
  ...
  it('marks selected chats as read', async () => {
    ...
    await waitFor(() => {
      ...
      expect(Alert.alert).toHaveBeenCalledWith('Успешно', 'Выбранные чаты отмечены как прочитанные');
    });
  });
  ...
  it('filters chats by search query', async () => {
    const { getByPlaceholderText, getByText, queryByText } = render(<ChatListScreen />);

...
} from "../../styles/screens/profile/TripsScreen.styles";
  ...
  const filteredTrips = useMemo(() => {
    ...
    if (currentFilter.status !== "all") {
      filtered = filtered.filter(
        (trip) => trip.status === currentFilter.status,
      );
    }
        ...
        {filteredTrips.length === 0 ? (
          ...
          <>
            {filteredTrips.map((trip) => (
              <View
                key={trip.id}
                style={[styles.carItem, dynamicStyles.carItem]}
              >
                    ...
                    >
                      <Text style={styles.statusText}>
                        {trip.status === "completed"
                          ? t("client.trips.status.completed")
                          : trip.status === "cancelled"
                            ? t("client.trips.status.cancelled")
                            : t("client.trips.status.scheduled")}
                      </Text>
                    </View>
                  </View>
                </View>

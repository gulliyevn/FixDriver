...
import {
  PaymentHistoryScreenStyles as styles,
  getPaymentHistoryScreenStyles,
} from "../../styles/screens/profile/PaymentHistoryScreen.styles";
import PaymentHistoryFilter, {
  PaymentFilter,
} from "../../components/PaymentHistoryFilter";
...
const PaymentHistoryScreen: React.FC<PaymentHistoryScreenProps> = ({
  ...
  const getScreenTitle = () => {
    return isDriver
      ? t("driver.paymentHistory.title")
      : t("client.paymentHistory.title");
  };

  const getEmptyStateTitle = () => {
    return isDriver
      ? t("driver.paymentHistory.noPayments")
      : t("client.paymentHistory.noPayments");
  };

  const getEmptyStateDescription = () => {
    return isDriver
      ? t("driver.paymentHistory.emptyDescription")
          ...
          <>
            {filteredTransactions.map((transaction) => {
              const title = transaction.translationKey
                ? (() => {
                    if (transaction.translationParams?.packageName) {
                      ...
                      const packageName =
                        transaction.translationParams.packageName.split("_")[0];
                      const translatedPackageName = t(
                        `premium.packages.${packageName}`,
                        { defaultValue: packageName },
                      );
                      return t(transaction.translationKey, {
                        ...transaction.translationParams,
              ...
              return (
                <View
                  key={transaction.id}
                  style={[styles.paymentItem, dynamicStyles.paymentItem]}
                >
                    ...
                    <View style={styles.paymentInfo}>
                      ...
                      <View style={styles.paymentDetails}>
                        ...
                        >
                          ...
                          • {formatTime(new Date(transaction.date), language)}
                        </Text>
                      </View>
                    </View>
                        ...
                        <Text style={styles.statusText}>
                          {transaction.status === "completed"
                            ...
                            : transaction.status === "pending"
                              ? t("client.paymentHistory.status.pending")
                              : t("client.paymentHistory.status.failed")}
                        </Text>
                      </View>
                    </View>
                  </View>
                  {transaction.description && (
                    <Text
                      style={[
                        ...
                        dynamicStyles.paymentDescription,
                      ]}
                    >
                      {transaction.description}
                    </Text>

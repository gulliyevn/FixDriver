...
const ResidenceScreen: React.FC<ResidenceScreenProps> = ({
        ...
        { text: t("profile.cancel"), style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            if (success) {
              ...
              Alert.alert(
                t("common.success"),
                t("profile.residence.deleteSuccess", { title: address.title }),
              );
            } else {
              Alert.alert(
                t("common.error"),
                t("profile.residence.deleteError"),
              );
            }
          },
        },
  ...
  const handleSetDefault = async (address: Address) => {
    ...
    if (success) {
      ...
      Alert.alert(
        t("common.success"),
        t("profile.residence.setDefaultSuccess"),
      );
    } else {
    ...
    try {
      if (modalMode === "add") {
        ...
        if (success) {
          ...
          Alert.alert(
            t("common.success"),
            t("profile.residence.updateSuccess"),
          );
        } else {
      ...
      >
        {loading ? (
          ...
          addresses.map((address) => (
            <View
              key={address.id}
              style={[styles.addressItem, dynamicStyles.addressItem]}
            >
              <View style={styles.addressHeader}>
                <View style={styles.addressInfo}>
                  <Text
                    style={[styles.addressTitle, dynamicStyles.addressTitle]}
                  >
                    {address.title}
                  </Text>
                  ...
                  {address.category && (
                    <Text
                      style={[
                        ...
                        dynamicStyles.addressDescription,
                      ]}
                    >
              ...
              {address.isDefault && (
                <View style={styles.defaultBadge}>
                  <Text style={styles.defaultText}>
                    {t("profile.residence.defaultBadge")}
                  </Text>
                </View>
              )}
            </View>

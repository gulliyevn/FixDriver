...
const TimeSchedulePage: React.FC<TimeSchedulePageProps> = ({
  ...
  const isSmooth = useMemo(
    () => state.switchStates.switch2 === true,
    [state.switchStates.switch2],
  );

  const isWeekdaysMode = useMemo(
    () => !isSmooth && state.switchStates.switch3 === true,
    [isSmooth, state.switchStates.switch3],
  );
  ...
  const handleSaveAndNext = async () => {
    ...
    const hasAnyTime =
      Object.values(state.times.fixed).some((time) => time) ||
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.switchesContainer}>
        ...
        />
        <View
          style={isSmooth ? styles.switchToggleDisabled : undefined}
          pointerEvents={isSmooth ? "none" : "auto"}
        >
          <SwitchToggle
      ...
      <View style={styles.switchLabelsContainer}>
        <Text style={[styles.switchLabel, { color: colors.textSecondary }]}>
          {state.switchStates.switch1
            ? t("common.thereAndBack")
            : t("common.there")}
        </Text>
        <Text style={[styles.switchLabel, { color: colors.textSecondary }]}>
        ...
        <Text
          style={[
            styles.switchLabel,
            {
              ...
              opacity: isSmooth ? 0.5 : 1,
              width: 120,
              textAlign: "center",
            },
          ]}
        >
          {state.switchStates.switch3
            ? t("common.weekdays")
            : t("common.fixed")}
        </Text>
      </View>
      ...
      {containers.map((container, index) => {
        ...
        const weekdayDepartureTime = getDepartureTime(
          index,
          state.times,
          container.color,
          isSmooth,
        ...
        const weekendDepartureTime = getDepartureTime(
          index,
          state.times,
          container.color,
          isSmooth,
        ...
        return (
          <ScheduleContainer
            ...
            onFixedTimeChange={(time) => {
              console.log(
                `📅 Изменение фиксированного времени для контейнера ${index}:`,
                {
                  time,
                  isSmooth,
                  isWeekdaysMode,
            ...
            onWeekdayTimeChange={(time) => {
              console.log(
                `📅 Изменение времени будней для контейнера ${index}:`,
                {
                  time,
                  isSmooth,
                  isWeekdaysMode,
            ...
            onWeekendTimeChange={(time) => {
              console.log(
                `📅 Изменение времени выходных для контейнера ${index}:`,
                {
                  time,
                  isSmooth,
                  isWeekdaysMode,
            ...
            onDayTimeChange={(dayKey, time) => {
              console.log(
                `📅 Изменение времени дня ${dayKey} для контейнера ${index}:`,
                {
                  time,
                  isSmooth,
                  isWeekdaysMode,
              ...
              if (!isSmooth && state.switchStates.switch3) {
                console.log(
                  `📅 Будни/Выходные режим - Изменение времени для контейнера ${index}:`,
                  {
                    time,
                    isWeekdaysMode,
                    currentTimes: state.times,

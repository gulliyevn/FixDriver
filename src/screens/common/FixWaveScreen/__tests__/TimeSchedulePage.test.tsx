...
const mockUseLanguage = require('../../../../context/LanguageContext').useLanguage;

describe('TimeSchedulePage', () => {
  const mockOnNext = jest.fn();
  const mockOnBack = jest.fn();
  const mockState = {
    timeScheduleData: { date: new Date(), time: '', isRecurring: false, notes: '' },
  ...
  describe('Сценарии с остановками', () => {
    test('ДОП1 по дороге - должен рассчитывать время корректно', () => {
      ...
      expect(stopContainer.props.fromCoordinate).toEqual(coordinatesWithStopOnRoute.stops[0]);
      expect(stopContainer.props.toCoordinate).toEqual(coordinatesWithStopOnRoute.to);
    });
    ...
    test('ДОП2 вне дороги - должен показывать --:--', () => {
      const coordinatesWithStopOffRoute = {
        ...
        stops: [
          ...
          { latitude: 60.0000, longitude: 30.0000 }  // Точка вне маршрута
        ]
      };
      ...
      expect(stopContainer2.props.fromCoordinate).toEqual(coordinatesWithStopOffRoute.stops[1]);

// Исправленный тестовый скрипт для проверки всех сценариев уровней и VIP
console.log('🧪 ИСПРАВЛЕННОЕ ТЕСТИРОВАНИЕ СЦЕНАРИЕВ УРОВНЕЙ И VIP\n');
...
console.log('\n🎯 СЦЕНАРИЙ 7: Проверка критических точек переходов');
console.log('='.repeat(50));

// Проверяем критические точки переходов
const criticalPoints = [
  { rides: 29, expected: '1.1', description: 'Последняя поездка 1.1' },
  { rides: 30, expected: '1.2', description: 'Первая поездка 1.2' },
  { rides: 69, expected: '1.2', description: 'Последняя поездка 1.2' },
  { rides: 70, expected: '1.3', description: 'Первая поездка 1.3' },
  { rides: 119, expected: '1.3', description: 'Последняя поездка 1.3' },
  { rides: 120, expected: '2.1', description: 'Первая поездка 2.1' },
  { rides: 179, expected: '2.1', description: 'Последняя поездка 2.1' },
  { rides: 180, expected: '2.2', description: 'Первая поездка 2.2' },
  { rides: 239, expected: '2.2', description: 'Последняя поездка 2.2' },
  { rides: 240, expected: '2.3', description: 'Первая поездка 2.3' },
];

criticalPoints.forEach(test => {
  const result = calculateLevelAndSubLevel(test.rides);
  const actual = `${result.level}.${result.subLevel}`;

...
export default class MockServices {
  ...
  static auth = {
    async login(email: string, password: string): Promise<AuthResult> {
      ...
      if (user) {
        ...
        if (password === 'password123') { // Mock password for testing
          // Save user to AsyncStorage for "remember me" functionality
          await MockData.setCurrentUser(user);
        ...
        // User not found in MockData.users, check AsyncStorage
        console.log('🔍 User not found in MockData, checking AsyncStorage...');
        const currentUserData = await AsyncStorage.getItem(STORAGE_KEYS.CURRENT_USER);
        if (currentUserData) {
          const storedUser = JSON.parse(currentUserData);
          if (storedUser.email === email) {
            // Add user to MockData.users if found in AsyncStorage
            MockData.addUser(storedUser);
            user = storedUser;
    ...
    async register(userData: any): Promise<AuthResult> {
      ...
      const newUser: User = {
        ...
        surname: userData.lastName || userData.surname || '',
      ...
      // Save user to AsyncStorage for "remember me" functionality
      await MockData.setCurrentUser(newUser);

      // In real app, this would be saved to database via gRPC
      console.log('🆕 Mock user registered and added to MockData:', newUser);
    ...
    async logout(): Promise<void> {
      ...
      await MockData.setCurrentUser(null);
  ...
  static users = {
    ...
    async getCurrent(): Promise<User | null> {
      try {
        console.log('🔍 MockServices.users.getCurrent() called');
        const user = await MockData.getCurrentUser();
        console.log('👤 MockServices.users.getCurrent() result:', user?.id || 'null');
        return user;
      } catch (error) {
        console.error('❌ Error in MockServices.users.getCurrent():', error);
        return null;
      }
    },

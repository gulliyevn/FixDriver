...
// Navigation types are unused here after onBack change
import { BalanceScreenStyles as styles, getBalanceScreenStyles, getBalanceScreenColors } from './styles/BalanceScreen.styles';
import { 
  mockQuickAmounts 
...
// import { createCVVStickAnimation } from '../../../../../shared/animations';
import BalanceCardDecoration from './BalanceCardDecoration';
import BalanceTopUpHistory from './BalanceTopUpHistory';

// Temporary shims; replace with shared hooks
import { useBalance, useDriverBalance, DriverBalanceContextType } from './shims/hooks';
import { useAuth } from '../../../../../core/context/AuthContext';
import { formatBalance } from '../../../../../shared/utils/formatters';
// Temporary visuals
const getPackageIcon = (_: any) => 'card';
const getPackageColor = (_: any) => '#3B82F6';
// Временная заглушка для usePackage
const usePackage = () => ({ currentPackage: 'free' as const });

// Временная заглушка для createCVVStickAnimation
const createCVVStickAnimation = (refs: any, setShowCVV: any) => (showCVV: boolean) => {
  // Простая заглушка анимации
  setShowCVV(!showCVV);
};

// Импорты стилей из локального файла стилей
import {
  balanceCardAnimated,
  balanceCardFrontRow,
  cashbackText,
  ...
  flipIconColor,
  backIconColor,
} from './styles/BalanceScreen.styles';
...
type BalanceScreenProps = { onBack: () => void };
...
const BalanceScreen: React.FC<BalanceScreenProps> = ({ onBack }) => {
  ...
  const handleWithdraw = (amount?: string) => {
    if (amount) {
      ...
      // fallback: открываем модал пополнения как ввод суммы
      setTopUpAmount('');
      setTopUpModalVisible(true);
    }
  };
  ...
  const handleWithdrawAmount = async (amount: number) => {
    ...
    if (withdrawBalance) {
      ...
      if (success) {
        Alert.alert(t('client.balance.success'));
      } else {
  ...
  const handleUseCashback = () => {
    ...
    if (cashbackNum < 20) {
      ...
      Alert.alert(t('client.balance.withdraw'), t('client.balance.useCashbackConfirm'));
    } else {
          ...
          { text: t('client.balance.cancel'), style: 'cancel' },
          {
            ...
            onPress: async () => {
              try {
                ...
                Alert.alert(t('client.balance.error'));
              }
            }
          }
        ]
  ...
  const handleFakeStripePayment = async () => {
    ...
    try {
      ...
      Alert.alert(t('client.balance.paymentSuccess'));
    } catch (error) {
        ...
        <View style={mainBalanceContainer}>
          <View style={cardContainerWithDynamicWidth(screenWidth)}>  
            ...
            >
              ...
              <BalanceCardDecoration isDark={isDark} packageType={currentPackage as any} />
            ...
            >
              ...
              <BalanceCardDecoration isDark={isDark} packageType={currentPackage as any} isBackSide={true} />

...
export const useVIPTimeTracking = (isVIP: boolean) => {
  const { addEarnings, resetEarnings } = useBalance();
  ...
  const performDayCheck = useCallback(async () => {
      ...
      if (today !== vipTimeData.currentDay) {
        ...
        if (vipTimeData.isCurrentlyOnline) {
          ...
          const previousDayEarnings = 0; // TODO: Get actual earnings from balance
          ...
          console.log(`[VIPTimeTracking] День завершен: ${vipTimeData.currentDay}, часы: ${previousDayStats.hoursOnline.toFixed(1)}, поездки: ${previousDayStats.ridesCount}, заработок: ${previousDayEarnings.toFixed(1)} AFc`);
  ...
  const startOnlineTime = useCallback(() => {
    ...
    driverStatusService.setOnlineStatus('driver_id', true);
  }, [isVIP, vipTimeData]);
  ...
  const stopOnlineTime = useCallback(() => {
    ...
    // Уведомляем DriverStatusService о изменении статуса
    driverStatusService.setOnlineStatus('driver_id', false);
  }, [vipTimeData]);

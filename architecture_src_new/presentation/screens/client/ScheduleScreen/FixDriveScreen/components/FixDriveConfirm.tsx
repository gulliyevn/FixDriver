...
const FixDriveConfirm: React.FC<FixDriveConfirmProps> = ({ 
  ...
  const { t } = useLanguage();
  // Простые стили для компонента
  const styles = {
    container: { flex: 1, backgroundColor: colors.background },
    flatListContainer: { flex: 1 },
    driversList: { paddingVertical: 8, paddingBottom: 20 },
    driverItem: {
    ...
    avatarContainer: { position: 'relative' as const, marginRight: 16 },
    avatar: {
      width: 48,
      height: 48,
      borderRadius: 24,
    ...
    driverMainInfo: { flex: 1, marginLeft: 16 },
    nameRatingRow: {
      flexDirection: 'row' as const,
    ...
    premiumIcon: { marginLeft: 4, opacity: 0.7 },
    ratingText: {
      fontSize: 18,
      color: '#10B981',
    ...
    expandableContent: { overflow: 'hidden' as const },
    tripsContainer: { marginVertical: 8 },
    tripItem: {
      flexDirection: 'row' as const,
      alignItems: 'center' as const,
      paddingVertical: 8,
